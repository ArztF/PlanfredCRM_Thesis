...
\end{lstlisting}
In dem oben gezeigten Fall wurde die sendStatus Funktion überschrieben und es wird von nun an eine eigens erstellte Response zurückgegeben. Die überschrieben Funktion kann nun auf folgende Weise aufgerufen werden.
\begin{lstlisting}
...
\end{itemize}
In dem Fall, das man mittels next einen Fehler aufrufen möchte, allerdings bereits eine response an den Client zu schicken begonnen hat, beendet Express die Verbindung und lässt den request fehlschlagen.
...
\end{verbatim}
Man hat nun auch die Möglichkeit weitere Optionen fürs Debuggen zu definieren.
\begin{center}
    \begin{tabular}{||c c||} 
        \hline
